                ...
                @foreach (get_user_notification_list() as $value)
                    <div class="toggle-container">
                        <label class="switch">
                            @php
                                $checked =
                                    isset($notification[$value['id']]) && $notification[$value['id']] ? 'checked' : '';
                            @endphp
                            <input type="checkbox" class="switch-notification" name="{{ $value['id'] }}"
                                id="{{ $value['id'] }}" value="1" {{ $checked }}>
...
@section('scripts')
    <script>
        var allChecked = $('.user__content').find('.switch-notification:checked').length === $('.user__content').find(
            '.switch-notification').length;
        $('.turn-all-off').text(!(allChecked) ? 'Turn All On' : 'Turn All Off');

        $('body').on('click', '.turn-all-off', function() {
            var _this = $(this);
            // Check if all checkboxes are checked
            var allChecked = _this.closest('.user__content').find('.switch-notification:checked').length === _this
                .closest('.user__content').find('.switch-notification').length;
            if (allChecked) {
                _this.closest('.user__content').find('.switch-notification').prop('checked', false).trigger(
                    'change');
                _this.text('Turn All On'); // Change button text
            } else {
                _this.closest('.user__content').find('.switch-notification').prop('checked', true).trigger(
                'change');;
                            ...
                            if (errors.hasOwnProperty(field)) {
                                ...
                                errorMessages.forEach(function(message) {
                                    $('input[name="' + field + '"]').after(
                                        '<p class="field-error mt-2 text-danger">' +
                                        message + '</p>')
                                });
                            }
                        }
